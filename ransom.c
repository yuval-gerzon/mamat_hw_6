...
void checkMagazine(int magazine_count, char** magazine, int note_count, char** note) {
	///struct eyvar_reshima {/*int* prev, */char* word, bool used, int* next} //this is an element in the linked list
	struct reshima {
		char* word[MAX_SIZE]
		bool used[MAX_SIZE]	
	}
	struct reshima mag_word_list
	///reshima = struct eyvar_reshima*[..._count]
	
	for (int i=0, i<magazine_count, i++){
		mag_word_list.word[i] = magazine[i]; ///TODO: make sure that magazine is linked the right way
		mag_word_list.used[i] = NOT_USED;
	}
	
	for (int k=0, k<note_count, k++){
		for (int l=0, l<magazine_count, l++){
			if ( !(strcmp(*note[k],*mag_word_list.word[l])) && (mag_word_list.used[i] == NOT_USED) ) {
			mag_word_list.used[i] = USED;
			continue
			}
			else{
				printf("no"); 
